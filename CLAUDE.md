...
### Sprint Campaign-Cleanup: eine bewusste Architektur-Umkehr, Status-Glimpse als ein Vokabular

Drei kleine Commits am Campaign/Phase-View (Desktop): Omnibusse wieder inline ausklappbar, Skull-Sterne in der Listenzeile durch Häkchenfarbe ersetzt, Omnibus-Ø-Rating wieder entfernt. Base `adce6af` → HEAD `39400d6`. Keine Fallstricke — die übertragbare Lehre ist prozessual.

- **Eine frühere Architekturentscheidung bewusst umkehren ist legitim — aber sie muss als solche benannt werden.** Der Omnibus-Sub-Items-Sprint hatte inline-Expansion in der Phasenliste bewusst gegen „ein Klick öffnet das Dossier, dort die CONTENTS-Liste" getauscht (dokumentiert: „sub-items do NOT appear as their own rows in Archive/Phase lists"). Tim wollte die Expansion zurück. Das ist kein Bug-Fix und kein Zurückrudern aus Versehen — es ist eine geänderte Anforderung. Beim Bauen NICHT so tun, als sei die alte Entscheidung falsch gewesen; sie war für ihren Zeitpunkt richtig. `BookRow` ruft sich jetzt rekursiv für die Kinder auf (Rekursionstiefe hart auf 2 durch die Daten begrenzt — Omnibus-Kinder haben nie selbst `contents`), verschachtelte `<button>`-Elemente vermieden durch Kinder-Block als SIBLING außerhalb des äußeren Buttons.

- **`BookRow` bleibt router- und context-frei — `getEntryProgress` wird als Prop durchgereicht, nicht via `useArchiveData()` importiert.** Um die Kinder aufzulösen, braucht die rekursive `BookRow` die Ableitungsfunktion. Statt den Context direkt in `BookRow` zu ziehen (was die „router-free presentational component"-Eigenschaft aus dem Book-Detail-Sprint gebrochen hätte), reicht `PhaseDetail` `getEntryProgress` als Prop durch. Reines Prop-Drilling, kein neuer Kopplungspunkt — die Komponente bleibt in jedem Mount wiederverwendbar.

- **Der Status-Glimpse in der Liste ist EIN Vokabular — Farbe des Häkchens trägt die Bedeutung, nicht ein zweites Icon daneben.** Erst wurde ein Ø-Rating-Symbol (Kreis+Strich) am Omnibus eingeführt und die Skull-Sterne am Einzelbuch durch Häkchenfarbe ersetzt (grün=gelesen, gold=gelesen&bewertet); dann wurde das Ø-Rating am Omnibus wieder ganz entfernt, weil zwei Rating-Aussagen (Ø 4.0 neben 3/3-Fortschritt) in einer Zeile als konkurrierende Zahlen gelesen wurden. Endzustand: die Listenzeile zeigt Status über Häkchenfarbe (Einzelbuch) bzw. Häkchen-statt-Zähler bei Vollständigkeit (Omnibus), das volle interaktive Rating lebt NUR im Dossier. LEHRE: `size="sm"` Skull-Icons (20px) sind nicht als Totenkopf lesbar und die „X/5"-Zahl daneben dupliziert nur — eine Listenzeile braucht einen Status-Glimpse, keine zweite Detailanzeige. Detail gehört ins Dossier.

- **Ein Omnibus-Häkchen ist grün, nicht gold — weil es nach Entfernen des Ø-Ratings keine „bewertet"-Aussage mehr am Omnibus-Level gibt.** Gold bedeutet in der Listenzeile „gelesen UND bewertet". Am voll gelesenen Omnibus gibt es kein aggregiertes Rating mehr in der UI, also wäre Gold irreführend. Grün = gelesen, ohne Rating-Aussage. Die Farbe ist Vokabular, nicht Dekoration — sie muss konsistent dasselbe behaupten.

- **Datenschicht darf mehr liefern als die UI zeigt — das ist kein toter Code.** Die Ø-Rating-Berechnung in `getEntryProgress` (Durchschnitt der bewerteten Kinder) blieb bestehen, obwohl das letzte Commit ihre einzige Anzeige entfernte. Sie ist korrekt, billig, und ein möglicher künftiger Abnehmer (Analysis-View, Service Record) findet die Ableitung schon fertig vor. Nur eine im Repo referenzierte, nirgends gerenderte JSX-Komponente (`AverageRatingIcon`) wurde entfernt — ungenutzte Render-Funktion = toter Import/Build-Warnung; ungenutzter Rückgabewert einer zentralen Ableitung = Reserve. Der Unterschied entscheidet, ob etwas raus muss.

### Sprint Chronicle-Interview: Fidelity-Fix, Opus-Wechsel, schwebendes Interview-Modal — und drei Fehldiagnosen

Ein Sprint über den Context-Drop/Chronicle-Pfad: das Aufblähen gestoppt, auf Opus 4.8 gewechselt, ein vierteiliges Interview-Feature gebaut (Modell fragt bei fraglichen Namen/dünnen Strängen nach), plus ein hart erkämpfter Bugfix. Base `a84d99f` → HEAD `144f5c1` (9 Commits). Die übertragbaren Lehren:

- **Eine feste Ziellänge im Schema ZWINGT das Modell zum Halluzinieren — Länge muss dem Input folgen.** Die `resonance`-Beschreibung befahl „2-3 paragraphs ... do not compress it ... aim for 2-3 paragraphs of 2-4 sentences each". Bei einem dünnen APPEND (eine Zeile Szenennotiz) gehorchte das Modell und erfand zwei Absätze „dramatic irony", die in keinem Rohtext standen. Der Fehler war NICHT das Modell, sondern die Anweisung: ein Längenziel gilt für jedes Modell, Sonnet wie Opus. Fix: „FIDELITY OVER LENGTH ... output length must follow the input ... never pad a thin reflection to hit a length ... add NOTHING they did not gesture at". `standout_moments` `minItems` 1→0 (ein dünner Braindump darf null Standouts haben). LEHRE: bei generativen Feldern nie eine feste Ziellänge vorgeben — das ist ein Auffüll-Befehl. „Treue vor Länge, Länge folgt Input" statt einer Zahl.

- **Ein mis-transkribierter Eigenname ist KEIN Modellfehler und darf nicht per Modell „korrigiert" werden.** „Cepharil" (statt „C'tan Shard") kam aus dem Wispr-Flow-Diktat, nicht vom Modell — das Modell übernahm treu, was im Rohtext stand. Ein stärkeres Modell hätte es NICHT eigenmächtig fixen sollen: bei zehntausenden obskuren 40k-Eigennamen würde ein Modell, das Namen „korrigiert", auch richtige Namen zerstören, die es nicht kennt. Ein nachgelagertes „Audit-LLM", das 40k-Trivia raten müsste, verschiebt das Problem nur auf eine unzuverlässigere Ebene. Die robuste Lösung ist der Mensch im Loop: das Modell MARKIERT Unsicherheit (was es gut kann) und fragt zurück, der Nutzer entscheidet (er kennt die Nomenklatur). LEHRE: bei Faktenwissen, das das Modell nicht zuverlässig hat, nie das Modell raten lassen — Unsicherheit markieren und den Menschen fragen.

- **Opus 4.8 für die Chronicle-Generierung, weil das Volumen winzig und die Treue kritisch ist.** Sonnet 4.6 war Legacy (Sonnet 5 seit Juni 2026). Der Call läuft ein paar Mal im Monat (ein Braindump pro fertigem Buch), also sind Token-Kosten irrelevant und die Modellqualität bei „treu bleiben, nicht auffüllen" ist der einzige Faktor. `MODEL = 'claude-opus-4-8'`. LEHRE: bei seltenen, qualitätskritischen Calls ist die Modellwahl eine reine Qualitätsfrage — Kosten sind kein Argument bei Single-User-Volumen.

- **Ein `structure`-Call, der einen Namen für fraglich hält, lässt die betroffenen Beats aus den Standouts — das war die Wurzel des Standout-Bugs.** Der eigentliche Bug (drei Fehl-Fixes lang): Sobald der erste Call correction-Fragen erzeugte (Cepharil/Katanen), gab er `standout_moments: []` zurück — Opus hält Beats, die an unsicheren Namen hängen, zurück, bis die Namen geklärt sind. Verifiziert wurde das erst durch einen DB-Query (`SELECT chronicle->'standout_moments' ...` zeigte `[]` direkt nach dem Structure-Call), nicht durch Raten. Fix: der RESOLVE-Call (nach Namensklärung) LEITET die Standouts frisch ab — „DERIVE standout_moments freshly ... this is the one place you SHOULD produce standouts even though the first pass left them empty". LEHRE: wenn ein LLM ein Feld unerwartet leer lässt, kann die Ursache ein FRÜHERER Schritt sein, nicht der offensichtliche letzte — die gespeicherten Daten zeigen, wo es wirklich leer wurde.

- **Drei Commits an der falschen Stelle, weil die Ursache vermutet statt gegen die Daten geprüft wurde.** Die Standout-Fehldiagnose lief so: (1) Prompt-Fix am resolve-Call „preserve standout_moments" — half nicht (Modell ignorierte es). (2) deterministischer Merge-Block, der die Original-Standouts im Code festschrieb — machte es SCHLIMMER, weil das Original selbst leer war, also fror er die Leere ein. (3) erst ein DB-Query zeigte: leer schon nach dem ersten Call. Alle drei Fixes beschuldigten den resolve-Call, obwohl der erste Call die Ursache war. Das verletzte die projekteigene Regel „state evidence before writing a prompt" drei Mal. LEHRE (die schärfste dieses Sprints): bei einem Datenfluss-Bug ZUERST die gespeicherten Daten an jeder Stufe inspizieren (hier: ein `execute_sql` auf die `chronicle`-Spalte), DANN die Stufe fixen, wo es nachweislich schiefgeht. Ein Fix ohne diesen Blick ist eine Vermutung, und drei Vermutungen kosten mehr als ein Query.

- **Prompt-basierte „Erhaltung" strukturierter Felder ist bei LLM-Edits unzuverlässig — aber die Alternative ist nicht immer Code.** Zwischenzeitlich war der Plan, `standout_moments`/`verdict_line` deterministisch im Code aus dem Original zu übernehmen (statt dem Modell zu vertrauen). Das war richtig für den ANGENOMMENEN Bug (Modell löscht bestehende Standouts), aber falsch für den ECHTEN Bug (Standouts waren nie da). Die finale Lösung ließ das Modell die Standouts ERZEUGEN, weil das der korrekte Schritt war. LEHRE: „trau dem Modell nicht, mach's im Code" ist ein gutes Muster — aber nur, wenn die Datenquelle für den Code-Weg tatsächlich die richtigen Daten enthält. Erst die Ursache beweisen, dann Muster wählen.
...
### Sprint Strategium-Constellation-Rework: Radius-Fraktion einer schmalen Box kollabiert den Cluster

Tims erster Live-Test des Strategium-Meta-Map (Screenshot) zeigte den Chaos-Cluster als überlappenden Punktklumpen in der Boxmitte, während Imperium seine Box gut nutzte. Sechs Commits ersetzten die Drei-Boxen-Architektur durch einen gemeinsamen Sternenraum. Zwei übertragbare Lehren.

- **Ein Radius, der aus der Box-HÖHE abgeleitet wird, kollabiert in einer schmalen Box — der Fehler steckt in der Kopplung, nicht in einem einzelnen Wert.** `radiusFor` skalierte mit `boxHeight` (0.075–0.12), aber `boxWidths` gab Chaos (nur ~5 Top-Level-Nodes) eine ~16%-Sliver-Breite. In `layoutBox` wurde `availableX = boxWidth/2 − BOX_PADDING − maxNodeRadius` dadurch ≤ 0, der isotrope Ellipsen-Fit-Scale kollabierte den gesamten Cluster auf einen Punkt in der Boxmitte — die Höhe blieb komplett ungenutzt. Der Bug war weder in `radiusFor` noch in `boxWidths` einzeln sichtbar, sondern nur in ihrem Zusammenspiel: zwei unabhängig plausible Formeln, deren Kopplung bei einem schmalen Seitenverhältnis reißt. REGEL: wenn eine Geometrie-Formel eine Dimension (Höhe) nutzt, um eine ANDERE Dimension (Breite) zu begrenzen, mit realistischen Extremwerten (schmalste erwartete Box) durchrechnen, bevor es als "funktioniert" gilt — nicht nur mit dem größten/typischsten Fall.

- **Eine bestehende CLAUDE.md-Pre-Build-Constraint kann durch Live-Test bewusst obsolet werden — das ist kein Rückbau, sondern eine Revision.** „Box size proportional to alliance faction count" stand explizit in den Strategium-Pre-Build-Constraints (2026-07-20) und wurde in genau dem Sprint gebaut, der jetzt den Bug zeigte. Statt die Formel zu patchen, entschied Tim nach dem Live-Screenshot: Boxen komplett raus, ein gemeinsamer Raum, Allianz nur über Farbe. Die alte Constraint wurde im Code-Kommentar (`strategiumLayout.js`) und in der Constraint-Sektion unten explizit als „deliberate reversal" markiert, nicht stillschweigend ersetzt. REGEL: eine dokumentierte Vorab-Entscheidung, die sich im ersten echten Live-Test als falsch erweist, wird korrigiert UND die Korrektur wird als bewusste Revision benannt (mit Grund) — sonst liest ein späterer Bauer die alte Constraint als noch gültig.

Sechs atomare Commits (`04be77d` Layout-Lib-Neuschrieb + Geometrie, `320e64a` Konstellations-Optik, `4d36313` Hook-API + Collide-Retune, `c1bc8ab` Top-3-Label-Policy, `5e71269` Nebel + Captions, `825159c` Sternenstaub) — jeder einzeln build-grün, App nach jedem Schritt lauffähig.

### Sprint Strategium-Flatten: derselbe Formfehler in Gegenrichtung, und eine Hierarchie, die nur wegen Platzmangel existierte

Tims zweiter Live-Test (Screenshots) zeigte: die Cluster nutzten den Raum kaum, obwohl der Klumpen-Bug behoben war — viel Leerraum zwischen den drei Clustern, winzige, kaum lesbare Icons. Vier Commits (`97ba4cb` sizePx-Prop, `6fbdf8c` Hierarchie-Flatten, `a46cb95` Fill-Scale-Fix + Größe). Zwei übertragbare Lehren.

- **Derselbe Formfehler wie beim Klumpen-Bug, nur mit der Richtung vertauscht.** Der Klumpen-Fix ersetzte den harten Box-Ellipsen-Kollaps durch `scale = Math.min(1, regionRadius / packedSpread)` — das verhindert Überlauf zuverlässig, WÄCHST aber nie, wenn ein Cluster (Chaos: wenige Nodes, kleiner natürlicher Pack) viel mehr Raum zur Verfügung hätte als er von sich aus braucht. Derselbe „nur eine Richtung bedacht"-Fehler wie beim ursprünglichen Box-Kollaps — nur diesmal Unternutzung statt Kollaps. Fix: `Math.max(1, (regionRadius * FILL_FRACTION) / packedSpread)` — Boden bei 1 (nie schrumpfen, das würde bei gleichbleibenden Radien die Non-Overlap-Garantie des Packers brechen), aber ungedeckelt nach oben (füllt den Raum). REGEL: ein Scale-Faktor, der mit `Math.min(1, ...)` ODER `Math.max(1, ...)` an 1 gekappt wird, deckt nur EINE Richtung (Schrumpfen ODER Wachsen) ab — bei jeder neuen Scale-Formel explizit prüfen, ob die andere Richtung (hier: ein Cluster mit reichlich ungenutztem Budget) im Zieldatensatz tatsächlich vorkommt, nicht nur den Überlauf-Fall testen.

- **Eine verschachtelte Sichtbarkeits-Hierarchie (Klick-zum-Aufklappen) kann eine reine Notlösung für Platzmangel gewesen sein, keine echte Informationsarchitektur.** Adeptus Astartes (9 Kapitel) und Chaos Undivided (7 Legionen) waren hinter Klick-Erweiterung versteckt, weil das alte Box-Layout keinen Platz für 41 gleichzeitig sichtbare Fraktionen hatte. Mit echtem Freiraum entschied Tim: alles flach zeigen, kein Klick-Mechanismus mehr. `buildFactionTree` (strategiumMap.js) verallgemeinert die alte „Ein-Kind-Umbrella kollabiert"-Regel (nur für die vier Chaos-Götter) zu „jede Umbrella ohne eigene Bücher kollabiert, unabhängig von der Kinderzahl" — Chaos Undivided (7 Kinder) verschwindet jetzt genauso wie die Ein-Kind-Fälle. Der ganze Expand/Collapse-State (expandedKey-Latch, Satelliten-Ring, Connector-Linien, Fokus-Dimmen) fällt ersatzlos weg — er existierte ausschließlich, um den Klick-Mechanismus zu bedienen. REGEL: wenn eine UI-Hierarchie ursprünglich aus Platzmangel entstand (nicht aus einer echten „gehört fachlich zusammen"-Beziehung), bei jeder Layout-Verbesserung neu fragen, ob der Platzmangel noch besteht — sonst überlebt ein Interaktionsmuster seinen eigentlichen Grund.

- **Tailwind-Größen-Token können keinen Laufzeit-Wert ausdrücken — ein `sizePx`-Escape-Hatch ist der allgemeine Fix, nicht Strategium-spezifisch.** `FactionSigil`/`FactionMark` kannten nur feste Tailwind-Klassen (`w-4 h-4` usw.) für eine feste Größen-Palette. Die Sterngröße im Meta-Map ist aber ein pro-Node berechneter Wert (Prominenz-abhängiger Radius) — eine Tailwind-JIT-Klasse kann einen zur Laufzeit erzeugten String wie `w-[${n}px]` nicht sehen, es gibt also keine Klasse dafür. Additiver `sizePx`-Prop (Inline-Style statt Token-Klasse) auf BEIDEN Komponenten (Sigil UND sein Fallback FactionMark, damit ein 404-Fallback nicht sichtbar die Größe wechselt) — kein bestehender Aufrufer betroffen. REGEL: sobald eine gemeinsame Komponente einen laufzeitberechneten statt einen aus einer festen Palette gewählten Wert braucht, verhindert eine reine Tailwind-Token-API das grundsätzlich — ein `sizePx`/`valuePx`-Inline-Style-Prop ist der Standard-Fix, nicht ein Sonderfall für diese eine Komponente.

### Sprint Strategium-Constellations: eine Suche, die "bis sie frei ist" weitersucht, terminiert irgendwann falsch

Nach dem Flatten (41 gleichzeitig sichtbare Fraktionen) folgten in schneller Folge vier fehlgeschlagene algorithmische Layout-Versuche: isotropes Kreis-Territorium pro Cluster (kollabierte einen schmalen Cluster auf einen Punkt), ein achsenweises Rechteck-Territorium + Fit-Scale (geriet aus dem Tritt, sobald das Packing footprint-bewusst wurde — Cluster wuchsen durch ihre Nachbarn hindurch), drei Runden reines `PACK_GAP`-Hochdrehen (14→18→40, hat die Geometrie nie repariert), und schließlich eine über alle Allianzen geteilte Kollisions-Liste (sollte Vermischung verhindern, ließ aber einen blockierten Node bei seinem EINEN festen Spiral-Winkel geradewegs durch einen fremden Cluster marschieren — der schlimmste Zustand von allen, Chaos-Fraktionen mitten im Imperium).

**Der gemeinsame Fehler-Shape hinter allen vieren:** jede Version war eine SUCHE ("vergrößere Radius/Scale, bis ein Kollisionscheck grün ist") — und eine Suche, die "einfach weitersucht, bis es passt", terminiert irgendwann an einer falschen Stelle. Kein einzelner Parameter war falsch; die Architektur selbst (Position durch Suche statt durch Festlegung) konnte diese Fehlerklasse strukturell nicht ausschließen.

**Der Ausweg — Positionen NACHSCHLAGEN statt SUCHEN.** `strategiumConstellations.js` (neu) kuratiert für jede Fraktion einen festen Platz in einem thematisch verwandten Sternbild (Adeptus Astartes' Kapitel, Inquisition+Grey Knights+Deathwatch, Aeldari–Drukhari als Paar usw.). `strategiumLayout.js`s `layoutConstellations` platziert jedes Mitglied in einer festen Gitterzelle innerhalb eines festen Blocks innerhalb einer festen Allianz-Region — drei geschachtelte, je disjunkte Partitionen, keine Iteration, kein Wachstums-Loop. Unregelmäßigkeit (deterministischer Hash-Jitter) und das Wackeln (`useStellarDrift`, 4px Amplitude) werden aus dem VORHER bewiesenen Restspiel jeder Zelle budgetiert — sie können es also nie aufbrauchen. Überlappung ist damit konstruktionsbedingt unmöglich, nicht mehr etwas, das ein Laufzeit-Check bei jeder neuen Geometrie erneut beweisen muss. REGEL: hängt eine Layout-Garantie davon ab, dass eine Suche irgendwann terminiert, wird sie irgendwann an der falschen Stelle terminieren — eine Partition, deren Disjunktheit strukturell (durch Konstruktion) gilt, ist einer Suche vorzuziehen, sobald die Suche zwei- oder dreimal am selben Symptom gescheitert ist.

**Robustheit bei künftigen DB-Änderungen — degradieren, nie crashen, nie schweigen.** Da die Sternbilder von Hand kuratiert sind, braucht es einen Plan für den Fall, dass sich der Datensatz ändert (neue Fraktion, entfernte Fraktion). `buildGroups` filtert kuratierte Mitglieder gegen den echten Katalog und sammelt alles Unzugeordnete in einem synthetischen `<allianz>-unaligned`-Sternbild; `validatePlan` prüft, ob jeder Block noch die richtige Slot-Zahl hat, und wirft — falls nicht — die GESAMTE Allianz auf eine generische Band-Platzierung zurück (`placeGeneric`, gleicher Gitter-Mechanismus, nur ohne die handkuratierte Form), plus ein dev-only `console.warn`. REGEL: bei handkuratierten Daten, die gegen einen sich ändernden externen Datensatz geprüft werden, immer einen automatischen Degradationspfad einbauen (nie nur eine Assertion, die crasht) — und den Degradationspfad LAUT machen (Warnung), nie still.

**Ein Wackel-Hook ist kein Physik-Ersatz, sobald die Positionen schon bewiesen kollisionsfrei sind.** `useStellarDrift` ersetzt den alten `useForceLayout` (d3-force: `forceCollide` + verankerte `forceX/forceY`) vollständig — nicht als Rewrite, sondern als andere Mechanik ohne gemeinsame Logik. Eine Collide-Force schiebt Nodes aktiv von ihrem Anker weg, sobald sie sich nähern — das hätte JEDE kuratierte Sternbild-Form zerstört, sobald zwei Sterne nah kämen, das exakte Gegenteil dessen, wofür kuratierte Platzierung da ist. Der neue Hook nutzt zwei inkommensurable Perioden (15000/21500ms) pro Node aus einem Fraktions-Hash, damit das Feld nie im Gleichtakt pulsiert, und begrenzt `|Δx|`/`|Δy|` strikt auf die Amplitude, die `layoutConstellations` bereits als Sicherheitsspiel reserviert hat. REGEL: sobald eine Physik-Simulation nur noch existiert, um Kollisionen aufzulösen, die die Layout-Schicht bereits ausschließt, ist sie nicht mehr nur überflüssig, sondern aktiv schädlich (sie kann die Garantie, die sie ersetzen sollte, selbst brechen).

## Strategium — Pre-Build Constraints (2026-07-20, teilweise überholt siehe unten)
...
- ~~**New runtime dependency: d3-force.**~~ **SUPERSEDED (Sprint Strategium-Constellations).** d3-force is removed entirely (dependency uninstalled). Positions are curated, fixed lattice cells (`strategiumLayout.js`'s `layoutConstellations`) plus a small deterministic wobble (`hooks/useStellarDrift.js`) — no simulation, no collision resolution at runtime, because the layout already proves non-overlap before the wobble hook ever runs.
- ~~**Physics anchor is a fixed code value, NOT a user setting.**~~ **SUPERSEDED, same sprint.** No physics anchor concept exists anymore. The successor is `WOBBLE_AMPLITUDE` (`strategiumLayout.js`, currently 4px) — still a fixed code constant, never a user setting, same underlying principle carried forward into the new architecture.
- ~~**Two distinct expansion states.**~~ / ~~**Auto-expand is in-situ, not promote-and-pin.**~~ **SUPERSEDED (Sprint Strategium-Flatten).** There is no expand/collapse mechanic left at all — every faction (including former "children" like individual chapters/legions) is its own always-visible flat node. See the Lessons Learned entry above: the click-to-expand hierarchy existed only to cope with the old boxed layout's lack of room, not as a real information architecture.
- ~~**Vectors follow-after-settle.**~~ **SUPERSEDED, same sprint.** There is no settling phase anymore — `useStellarDrift` returns correct positions from the first frame (anchors ARE the finished layout, wobble is a perpetual small drift, not a settle-then-stop simulation), so a recommendation vector's endpoint is always correct, no "wait for alpha to drop" logic needed.
- **Vectors need curved routing.** A pivot crosses cluster boundaries (correct — it leaves the alliance), but a straight line slashes through unrelated nodes at dense layouts. Use a gentle curved connector routing around nodes.
- **`prefers-reduced-motion` is mandatory.** Compute the final layout and snap to it, no animation, when the setting is on.
- ~~**Box size proportional to alliance faction count, not book count.**~~ **SUPERSEDED (Sprint Strategium-Constellation-Rework).** The per-alliance bounding boxes are gone entirely — see the Lessons Learned entry above. A count-proportional box width, combined with a node radius scaled off box HEIGHT, is exactly what collapsed the Chaos cluster to a point on first live test. Alliance membership is now carried by colour alone, in one shared canvas; node radius is a fixed global pixel size (`strategiumLayout.js`).
- **Sigil fallback required.** ~62 factions are sigil-NULL; the fan-out has holes without a fallback glyph.
- **Data prerequisites gate the frontend.** Grand-alliance mapping, `parent_faction` grouping, `prominence` signal, and sigil completion are a data sprint that must land first — already landed as of this rework.
